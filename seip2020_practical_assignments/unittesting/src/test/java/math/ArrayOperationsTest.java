...
public class ArrayOperationsTest {
  ...
  public void testFindPrimesInFile_validTestingCases_expectedOutputs() {
    int[] expectedValidSample = {3, 5, 7, 2, 2, 2};
    ...
    when(fio.readFile(filepath)).thenReturn(new int[]
      {3, 4, 5, 7, 0, 9, 6, 2, 0, 0, -2, 8, 0, 0, 4, 10, -4, 2, 2, 4});
  ...
  @Test
  public void testFindPrimesInFile_invalidTestingCases_expectedOutputs() {
    int[] expectedInvalidSample = {3, 5, 2, 2};
    filepath = new File("src/test/resources/invalid_sample.txt").getAbsolutePath();

    FileIO fio = mock(FileIO.class);
    when(fio.readFile(filepath)).thenReturn(new int[]
      {3, 4, 5, 0, 9, 6, -2, 0, 0, 8, 0, 4, 4, 2, 2, 999999999, 4, -5, 0});
  ...
  public void testFindPrimesInFile_validTestingCases_expectedOutputs() {
    int[] expectedValidSample = {3, 5, 7, 2, 2, 2};
    filepath = new File("src/test/resources/valid_sample.txt").getAbsolutePath();
    Assert.assertArrayEquals(expectedValidSample, arrayOps.findPrimesInFile(fileIo, filepath, myMath));
  }
  */
