# Unit Testing
A project for practicing Unit Testing.

## Getting Started
### Prerequisites
Install [Java](https://www.oracle.com/java/technologies/javase-downloads.html) and [Maven](https://maven.apache.org/), if you haven't already done.

### Installation
- #### Clone 
  ```git clone https://github.com/pvarouktsis/lab-assignment-2.git```
- #### Change Directory
  ```cd ./lab-assignment-2/seip2020_practical_assignments/```
- #### Unit Testing
  ```mvn test```
- #### JaCoCo Report
  ```mvn package jacoco:report```
- #### Open Report
  - Access it directly via GUI at  
  ```lab-assignment-2/seip2020_practical_assignments/unittesting/target/site/jacoco/index.html```  
  - MacOS via CLI  
  ```open ./unittesting/target/site/jacoco/index.html```  
  - Linux  via CLI  
  ```gnome-open|xdg-open|kde-open ./unittesting/target/site/jacoco/index.html```  

## Built with
- [Maven](https://maven.apache.org/) \- Dependency Management

## Version
- Unit Testing 1.0.0

## Authors
- Panagiotis Varouktsis


